...
1. 为 `ActionParser::validateAgainstSchema` 增加覆盖测试。
2. 为高危动作确认增加“本轮记住选择”机制，减少频繁重复输入。
3. 将 `Execute` 从黑名单策略逐步迁移为“命令模板白名单 + 参数化”。
4. 追加结构化日志（JSON Lines）：`trace_id/round_id/action_id/duration/token_usage`。
